...
export default {
    ...
    "watch": {
        ...
        "defaults": function(newVal,oldVal){
            ...
            if (self.checkable){
                ...
                if (newVal.length){
                    let key=newVal[0];
                    let item=self.groupTitleMap[key];
                    if (!item) return;
    ...
    "methods": {
        ...
        updateStatus () {
            ...
            const length = self.vehicleTreeList.length;
            if (!length) return;
            let len=Math.ceil(length/4000);
                ...
                for (let i=0;i<len;i++){
                    self.queryVehicle({
                        ...
                        json.forEach((item) => {
                            ...
                            self.$set(self.vehicleMap[item.vehicleId], "onlineStatus", item.onlineStatus);
                        });
                    });
                }
            ...
            if (data.groupType=="车辆"){
                ...
                return h("span",
                    ...
                    [
                        h("span",[
                            h("span", {
                                "class": "iconfont "+((data.onlineStatus==1 && data.channel && data.simCardNo )?"icon_video fs10":"icon_video fs10") ,
                                "attrs": {
                                    "title": ((data.onlineStatus==1 && data.channel && data.simCardNo )?"可播放":"不可播放")
                                },
                                "style": {
                                    "padding-right": "7px",
                                    "color": (data.onlineStatus==1 && data.channel && data.simCardNo )?"#00A854":"#999"
                                }
                            })
                        ]),
                        h("span",{
        ...
        "sortBy": function(vehicles){
            vehicles.sort(function(a,b){
                let val1 = a.onlineStatus;
                let val2 = b.onlineStatus;
                if (val1 > val2) {
                    return -1;
                } else if (val1 < val2) {
                    return 1;
